...
//updateMatchCount 缓存命中：相同 keyword 二次调用不重扫
TEST(UT_test_textedit_updateMatchCount, UT_test_textedit_updateMatchCount_CacheHit)
{
    Window *pWindow = new Window();
    pWindow->addBlankTab(QString());
    TextEdit *editor = pWindow->currentWrapper()->textEditor();
    ...
    editor->updateMatchCount(QString("hello"), Qt::CaseInsensitive);
    int sizeAfterFirst = editor->m_allMatchPositions.size();
    QString keywordCache = editor->m_countedKeyword;

    editor->updateMatchCount(QString("hello"), Qt::CaseInsensitive);
    int sizeAfterSecond = editor->m_allMatchPositions.size();

    ASSERT_EQ(sizeAfterFirst, 3);
    ASSERT_EQ(sizeAfterSecond, 3);
